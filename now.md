...
> **关键发现**: 项目实际实现远超 plan.md 中 Stage 1-6 的规划，包含大量计划外功能（聊天历史、个性系统、能力系统、多模态图片分析）。成长逻辑已迁移到 core/game/growth.py，任务难度自动判定、成长反馈 UI（浮动 +EXP 动画）、首次引导流程均已实现。未完成的 P2 项仅剩 EventBus 逐步替换 Qt Signal。
